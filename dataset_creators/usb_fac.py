...
DS_VARIANT = "factuality_classification"
...
for (split_name, split_examples) in zip(["train", "validation","test"],
    ...
    with jsonlines.open(f"{OUTPUT_ROOT}/{DS_VARIANT}/{split_name}.jsonl", "w") as w:
        for i, dp in enumerate(clusterwise_dps):
            newdp = dict()
            newdp['id'] = dp['id']
            newdp['summ_idx'] = dp['summ_idx']
            newdp['input_lines'] = dp['input_lines']
            newdp['summary_sent'] = dp['after_lines'][0]
            newdp['label'] = 0
            w.write(newdp)
            ...
            newdp['summ_idx'] = dp['summ_idx']
            newdp['input_lines'] = dp['input_lines']
            newdp['summary_sent'] = dp['before_lines'][0]
            newdp['label'] = 1
            w.write(newdp)
